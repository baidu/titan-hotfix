 ...
 *
 * <p>You do not need to use {android.support.v4.content.WakefulBroadcastReceiver}
...
public abstract class JobIntentService extends Service {
    ...
    static final class CompatWorkEnqueuer extends WorkEnqueuer {
        ...
        void enqueueWork(Intent work) {
            ...
            if (DEBUG) {
                Log.d(TAG, "Starting service for work: " + work);
            }
    ...
    final class CompatWorkItem implements GenericWorkItem {
        ...
        public void complete() {
            if (DEBUG) {
                Log.d(TAG, "Stopping self: #" + mStartId);
            }
            stopSelf(mStartId);
        }
    }
    ...
    public int onStartCommand(@Nullable Intent intent, int flags, int startId) {
        if (mCompatQueue != null) {
            ...
            if (DEBUG) {
                Log.d(TAG, "Received compat start command #" + startId + ": " + intent);
            }
            ...
            if (DEBUG) {
                Log.d(TAG, "Ignoring start command: " + intent);
            }
    ...
    public IBinder onBind(@NonNull Intent intent) {
        if (mJobImpl != null) {
            ...
            if (DEBUG) {
                Log.d(TAG, "Returning engine: " + engine);
            }
            return engine;
        } else {
            return null;
    ...
    void ensureProcessorRunningLocked(boolean reportStarted) {
        if (mCurProcessor == null) {
            ...
            mCurProcessor.executeOnExecutor(AsyncTask.SERIAL_EXECUTOR);
        }
    }
