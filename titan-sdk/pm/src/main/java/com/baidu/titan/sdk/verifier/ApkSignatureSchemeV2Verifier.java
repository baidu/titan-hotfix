...
public class ApkSignatureSchemeV2Verifier {
     ...
     * <p><b>NOTE: This method does not verify the signature.</b>
     */
    public static boolean hasSignature(File apkFile) throws IOException {
        RandomAccessFile apk = new RandomAccessFile(apkFile, "r");
        try {
            findSignature(apk);
            return true;
     ...
     * @throws IOException if an I/O error occurs while reading the APK file.
     */
    public static X509Certificate[][] verify(File apkFile)
            throws SignatureNotFoundException, SecurityException, IOException {
        RandomAccessFile apk = new RandomAccessFile(apkFile, "r");
        try {
            return verify(apk);
    ...
    private static X509Certificate[][] verify(RandomAccessFile apk)
        ...
        return verify(apk, apk.getFD(), signatureInfo);
    }
    ...
    private static void verifyIntegrity(
        ...
        if (apkSigningBlockOffset > CHUNK_SIZE_BYTES) {
            beforeApkSigningBlock =
                    new MemoryMappedFileDataSource(apk.getChannel(), 0, apkSigningBlockOffset);
            centralDir =
                    new MemoryMappedFileDataSource(apk.getChannel(), centralDirOffset, eocdOffset - centralDirOffset);
        } else {

            ByteBuffer beforeApkBuf = ByteBuffer.allocate((int) apkSigningBlockOffset);
            beforeApkBuf.order(ByteOrder.LITTLE_ENDIAN);
            try {
                apk.seek(0);
                apk.readFully(beforeApkBuf.array(), beforeApkBuf.arrayOffset(), beforeApkBuf.capacity());
            } catch (IOException e) {
                throw new SecurityException("Failed to get apk contents", e);
            }
            beforeApkSigningBlock = new ByteBufferDataSource(beforeApkBuf);


            ByteBuffer centralDirBuf = ByteBuffer.allocate((int) (eocdOffset - centralDirOffset));
            centralDirBuf.order(ByteOrder.LITTLE_ENDIAN);
            try {
                apk.seek(centralDirOffset);
                apk.readFully(centralDirBuf.array(), centralDirBuf.arrayOffset(), centralDirBuf.capacity());
            } catch (IOException e) {
                throw new SecurityException("Failed to get apk contents", e);
            }
            centralDir = new ByteBufferDataSource(centralDirBuf);
        }
    ...
    private static final class MemoryMappedFileDataSource implements DataSource {
         ...
         * @param size size (in bytes) of the region.
         */
        public MemoryMappedFileDataSource(FileChannel channel, long position, long size) {
            mChannel = channel;
